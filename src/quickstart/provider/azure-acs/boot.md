...
1: Find the fully qualified domain name (FQDN) for the Kubernetes master:
```
$ az acs list
...
2: Download the Kubeconfig from the master to your local machine, make sure to use the right SSH identity and master FQDN:
```
...
3: Point `kubectl` at the kubernetes configuration file by setting the `KUBECONFIG` environment value:
```
